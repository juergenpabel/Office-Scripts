...
The xtea_op must either be "encpryt" or "decrypt", or the script will just exit. The xtea_key_hex must contain a hex encoded 128bit key and the xtea_iv_hex a hex encoded 64bit initialization-vector. The to-be-en/decrypted data must be provided as a bas64 string.
